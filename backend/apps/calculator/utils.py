...
# --- Stronger Helper function to clean text ---
def clean_text(text: str) -> str:
    # 1. Add space between lowercase followed by uppercase
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text)
    # 2. Add space between letters and punctuation if missing
    text = re.sub(r'([.,!?()])([A-Za-z])', r'\1 \2', text)
    # 3. Add space between two lowercase/uppercase boundaries if no space
    text = re.sub(r'([a-zA-Z])([A-Z][a-z])', r'\1 \2', text)
    # 4. Add space before capital letters in the middle of a word (fallback)
    text = re.sub(r'([a-z])([A-Z])', r'\1 \2', text)
...
def analyze_image(img: Image, dict_of_vars: dict):
   ...
   # return answers
    for answer in answers:
        if isinstance(answer.get("result"), str):
            answer["result"] = clean_text(answer["result"])
        if isinstance(answer.get("expr"), str):
            answer["expr"] = clean_text(answer["expr"])
        if 'assign' in answer:
            answer['assign'] = True
        else:
            answer['assign'] = False
